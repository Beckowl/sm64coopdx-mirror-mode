...
local function disable_object_face_culling(o, modelID)
    local sharedChild = o.header.gfx.sharedChild
    modelID = modelID or obj_get_model_id_extended(o)

    if not cullingFixed[modelID] and sharedChild then
        disable_face_culling(sharedChild)

        cullingFixed[modelID] = true
    end
end
...
local function on_geo_process(node)
    ...
    if node.extraFlags & FLAG_CULLING_DISABLED == 0 then
        ...
        -- it seems some objects do not trigger HOOK_ON_OBJECT_SET_MODEL,
        -- so i need to fix them here
